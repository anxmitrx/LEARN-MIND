...
function SuggestedMentorItem({ mentor }: { mentor: any }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100 hover:border-indigo-200 transition-all">
      <Link to="/u/$uid" params={{ uid: mentor.uid }} className="flex items-center gap-3 flex-1 min-w-0">
      ...
      {user && user.uid !== mentor.uid && (
        <button
          ...
          title="Follow"
        >
          {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : isFollowing ? <Check className="w-4 h-4" /> : <Plus className="w-4 h-4" />}
        </button>
      )}
    </div>
  );
