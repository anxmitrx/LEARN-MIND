...
export function Navbar() {
  ...
  return (
    <header className="fixed top-[calc(var(--banner-height,0px)+1.5rem)] left-1/2 -translate-x-1/2 w-[95%] max-w-7xl z-50 rounded-full bg-white/20 backdrop-blur-lg border border-white/40 shadow-[0_8px_30px_rgba(0,0,0,0.04)] transition-[top] duration-200">
      <div className="w-full px-4 sm:px-6 lg:px-8 py-2 flex items-center justify-between gap-4 lg:gap-8">
            ...
            {links.map((l, i) =>
              l.subLinks ? (
                <div key={i} className="relative group flex items-center h-full">
                  <button className="flex items-center gap-1.5 py-2 text-[15px] font-semibold text-slate-700 hover:text-blue-600 transition-colors focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:outline-none cursor-pointer">
                    <span>{l.label}</span>
                ...
                l.to && (
                  <Link
                    ...
                    className="relative flex items-center gap-1.5 py-2 text-[15px] font-semibold text-slate-700 hover:text-blue-600 transition-colors data-[status=active]:text-blue-600 focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:outline-none whitespace-nowrap"
                  >
                    <span>{l.label}</span>
                    {l.badge && (
        ...
        <div className="flex justify-end items-center gap-3 shrink-0">
          ...
          <button
            onClick={() => openModal()}
            className="hidden md:flex items-center text-[15px] font-bold text-blue-600 hover:text-blue-800 transition-colors cursor-pointer mr-2"
          >
            Reserve Seat
          </button>
          ...
          <button
            ...
            className="grid h-10 w-10 place-items-center bg-slate-100 text-slate-700 xl:hidden rounded-full cursor-pointer focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:outline-none"
          >
