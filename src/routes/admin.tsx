...
function AdminComponent() {
  ...
  useEffect(() => {
    ...
    const unsubscribeRes = onSnapshot(
      resQuery,
      (snap) => {
    ...
    const unsubscribeConsult = onSnapshot(
      consultQuery,
      (snap) => {
    ...
    const unsubscribeWebinars = onSnapshot(
      webinarsQuery,
      (snap) => {
        if (snap.empty) {
  ...
  return (
    <main className="min-h-screen bg-transparent text-slate-800 relative">
      ...
      <div className="pointer-events-none absolute inset-0 -z-10 overflow-hidden select-none">
        <div
          className="absolute top-[10%] left-[10%] w-[35vw] h-[35vw] max-w-[400px] rounded-full bg-indigo-200/20 blur-3xl animate-pulse"
          style={{ animationDuration: "14s" }}
        />
        <div
          className="absolute bottom-[20%] right-[15%] w-[40vw] h-[40vw] max-w-[450px] rounded-full bg-purple-200/20 blur-3xl animate-pulse"
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="font-mono text-xs font-bold text-indigo-600/70 tracking-widest uppercase">
                  // SECURED ACCESS
                </span>
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          <div className="p-6 bg-white/40 backdrop-blur-xl border border-white/60 shadow-sm rounded-3xl flex items-center justify-between">
            <div>
              ...
              <h3 className="text-3xl font-display font-extrabold text-indigo-600 mt-1">
                {totalUsersCount}
              </h3>
            </div>
          ...
          <div className="p-6 bg-white/40 backdrop-blur-xl border border-white/60 shadow-sm rounded-3xl flex items-center justify-between">
            <div>
              ...
              <h3 className="text-sm font-display font-extrabold text-indigo-650 mt-2 leading-snug line-clamp-3">
                {getPopularTrack()}
              </h3>
            </div>
          ...
          <div className="p-6 bg-white/40 backdrop-blur-xl border border-white/60 shadow-sm rounded-3xl flex items-center justify-between">
            <div>
              ...
              <h3 className="text-xs font-display font-extrabold text-indigo-650 mt-2 leading-snug line-clamp-3">
                {getTopChallenge()}
              </h3>
            </div>
            <div className="p-3 bg-indigo-500/10 text-indigo-600 rounded-2xl shrink-0">
              ...
              {filteredReservations.length === 0 ? (
                <div className="text-center py-20">
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Waitlist entries will appear here once users submit reservations.
                  </p>
                </div>
              ) : (
                    ...
                    {filteredReservations.map((row) => (
                      ...
                      >
                        ...
                        <td className="p-4 text-xs">
                          ...
                          >
                            <span
                              className={`h-1.5 w-1.5 rounded-full ${row.status === "confirmed" ? "bg-emerald-500" : "bg-amber-500"}`}
                            ></span>
                            {row.status || "pending"}
                          </span>
                        </td>
                        <td className="p-4 text-xs font-semibold text-slate-550 capitalize">
                          {row.source || "website"}
                        </td>
            ...
            <div className="w-full overflow-x-auto rounded-2xl">
              {filteredQuizResults.length === 0 ? (
                <div className="text-center py-20">
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Quiz diagnostic responses will appear here as users submit them.
                  </p>
                </div>
              ) : (
                <table className="w-full text-left border-collapse">
            ...
            <div className="w-full overflow-x-auto rounded-2xl">
              {filteredConsultations.length === 0 ? (
                <div className="text-center py-20">
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Class 12 consultation bookings will appear here.
                  </p>
                </div>
              ) : (
                <table className="w-full text-left border-collapse">
            ...
            <div className="w-full overflow-x-auto rounded-2xl">
              <div className="p-4 flex justify-between items-center bg-indigo-50/20 border-b border-indigo-100/50">
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    Manage the content that appears on the website.
                  </p>
                </div>
                <div className="flex gap-2">
                  <button
              ...
              {filteredWorkshops.length === 0 ? (
                <div className="text-center py-20">
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Click "Seed Initial Data" to populate Firestore.
                  </p>
                </div>
              ) : (
                <table className="w-full text-left border-collapse">
            ...
            <div className="w-full overflow-x-auto rounded-2xl">
              <div className="p-4 flex justify-between items-center bg-indigo-50/20 border-b border-indigo-100/50">
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    Manage live events and recordings on the site.
                  </p>
                </div>
                <div className="flex gap-2">
                  <button
                  ...
                  <tbody>
                    {filteredWebinars.map((row, idx) => (
                      <tr
                        ...
                        className="border-b border-white/20 hover:bg-white/30 transition-colors duration-150 relative group"
                      >
                        <td className="p-4 text-xs font-bold text-slate-900 max-w-[200px] truncate">
                          {row.title || "N/A"}
                        </td>
                        <td className="p-4 text-xs font-semibold text-slate-700">
                          {row.presenter || "N/A"}
                        ...
                        <td className="p-4 text-xs">
                          ...
                          >
                            <span
                              className={`h-1.5 w-1.5 rounded-full ${row.status === "upcoming" ? "bg-emerald-500" : "bg-slate-500"}`}
                            ></span>
                            {row.status || "upcoming"}
                          </span>
                        </td>
                        <td className="p-4 text-right whitespace-nowrap">
                          ...
                          <button
                            onClick={() => {
                              if (!row.id) {
                                alert(
                                  "Please click 'Seed Initial Data' before editing to prevent other webinars from disappearing.",
                                );
                                return;
                              }
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-100 p-4 sm:p-6 bg-slate-50/50">
                <h3 className="font-display text-lg font-bold text-slate-900">
                  Edit{" "}
                  {editingRow.collection === "reservations"
                    ? "Reservation"
                    : editingRow.collection === "quiz_results"
                      ? "Quiz Result"
                      : "Consultation"}
                </h3>
                <button
