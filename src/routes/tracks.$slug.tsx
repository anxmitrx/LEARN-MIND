...
function TrackPage() {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative overflow-hidden bg-background">
        ...
        <div className="container relative mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:py-24">
          ...
          <div className="mt-8 grid items-center gap-12 lg:grid-cols-[1.15fr_0.85fr]">
            <div>
              <span className="inline-block bg-yellow px-3.5 py-1 text-xs font-display font-extrabold uppercase tracking-wider text-ink rounded-full">
            ...
            <div className="bg-background p-6 rounded-3xl shadow-xl shadow-[#3A3532]/5 border border-[#3A3532]/5">
      ...
      <section id="bento" className="bg-surface py-24">
        <div className="container mx-auto max-w-7xl px-4 sm:px-6">
          <div className="max-w-2xl">
            ...
            <h2 className="mt-3 font-display text-5xl font-bold leading-[0.95] tracking-wide text-ink sm:text-6xl">
              The <span className="bg-yellow px-2 rounded-lg">bento.</span>
            </h2>
          </div>
          ...
          <div className="mt-12 grid gap-5 lg:grid-cols-3">
              ...
              <ul className="mt-6 grid gap-3 sm:grid-cols-2">
                {track.topics.map((tp, i) => (
                  <motion.li
                    ...
                    className="flex items-center gap-3 bg-surface px-4 py-3 rounded-2xl border border-[#3A3532]/10"
                  >
                    <span className="grid h-9 w-9 shrink-0 place-items-center bg-yellow rounded-full">
                      <tp.icon className="h-4 w-4 text-ink" />
                    </span>
                    <span className="font-display text-sm font-bold text-ink">{tp.title}</span>
                  </motion.li>
                ))}
              </ul>
            </BentoCard>
              ...
              <ul className="mt-5 space-y-4">
                {track.outcomes.map((o, i) => (
                  <li key={i} className="flex items-start gap-3">
                    <span className="mt-0.5 grid h-6 w-6 shrink-0 place-items-center bg-background rounded-full shadow-sm">
            ...
            <BentoCard tone="ink" className="lg:col-span-3">
              ...
              <h3 className="mt-2 font-display text-3xl font-bold text-background">A typical workshop</h3>
              <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {track.agenda.map((a, i) => (
                  <div key={i} className="relative bg-[#3A3532] border border-[#8AA392]/20 p-5 rounded-2xl">
                    <span className="absolute -top-3 left-3 bg-yellow px-3.5 py-0.5 font-mono text-xs font-extrabold text-ink rounded-full">
                      {String(i + 1).padStart(2, "0")}
                    </span>
                    <div className="mt-2 font-mono text-xs text-yellow">{a.time}</div>
                    <h4 className="mt-1 font-display text-base font-bold text-background">{a.title}</h4>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {tracks.filter((t) => t.slug !== track.slug).map((t) => (
              <Link
                ...
                to="/tracks/$slug"
                params={{ slug: t.slug }}
                className="bento-card bg-background p-6 rounded-3xl shadow-md shadow-[#3A3532]/5"
              >
