...
const contentData: Record<
  string,
  { title: string; image: string; text: string[]; subtitle: string }
> = {
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-3xl mx-auto text-center">
            <Link
              to="/"
              hash="beyond"
              className="inline-flex items-center gap-2 text-indigo-600 font-medium mb-6 hover:text-indigo-800 transition-colors"
            >
              <ArrowLeft className="h-4 w-4" />
              Back to Programs
            </Link>
            ...
            <p className="text-xl md:text-2xl text-slate-600 max-w-2xl mx-auto">{data.subtitle}</p>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="mt-20 p-8 sm:p-12 bg-indigo-50 rounded-[2rem] border border-indigo-100 text-center">
              <h3 className="text-2xl font-display font-bold text-indigo-950 mb-4">
                Ready to step beyond the classroom?
              </h3>
              <p className="text-indigo-800/80 mb-8 max-w-xl mx-auto">
                Join our upcoming batch and get exclusive access to our {data.title.toLowerCase()}{" "}
                and much more.
              </p>
              <Link
                to="/"
                className="inline-flex items-center justify-center px-8 py-4 bg-indigo-600 text-white rounded-full font-bold shadow-lg hover:bg-indigo-700 hover:shadow-xl transition-all transform hover:-translate-y-1"
              >
                Apply Now
              </Link>
            </div>
