...
import { useWebinars, fetchWebinarBySlug } from "@/hooks/useWebinars";
...
export const Route = createFileRoute("/webinars/$slug")({
  ...
  loader: async ({ params }): Promise<{ webinar: Webinar }> => {
    const webinar = await fetchWebinarBySlug(params.slug);
    if (!webinar) throw notFound();
    return { webinar };
  },
  head: ({ loaderData }) => {
    const w = loaderData?.webinar;
    const title = w ? `${w.title} — Learn & Shine` : "Webinar — Learn & Shine";
    const desc = (w as any)?.description ?? "Learn & Shine mentoring webinar.";
    return {
      meta: [
        { title },
...
function WebinarPage() {
  const { webinar: track } = Route.useLoaderData() as { webinar: Webinar };
  const { webinars: tracks } = useWebinars();
  const isEngineering = false; // We can adapt this if needed later
  ...
  return (
    <main className="min-h-screen bg-transparent text-slate-800">
          ...
          <div className="mt-8 grid items-center gap-12 lg:grid-cols-[1.15fr_0.85fr]">
            <div>
              ...
              <p className="mt-6 max-w-xl text-lg text-slate-600 font-semibold">
                {(track as any).description || `Join ${(track as any).presenter || (track as any).hostName} for an exclusive session.`}
              </p>
              <div className="mt-8 flex flex-wrap gap-4">
                <MagneticButton size="lg" onClick={() => window.open(track.link, "_blank")}>
                  Register & Join
                </MagneticButton>
                <MagneticButton
                  variant="outline"
            ...
            <div className="bg-white/50 backdrop-blur-xl border border-white/60 shadow-[0_8px_32px_0_rgba(31,38,135,0.07)] rounded-3xl p-5 sm:p-6">
              ...
              <div className="mt-4 grid place-items-center">
                <SkillRadar data={(track as any).radar || []} />
              </div>
            </div>
          </div>
        </div>
      ...
      {((track as any).topics || (track as any).outcomes || (track as any).agenda) && (
      <section id="bento" className="bg-transparent py-24">
        <div className="container mx-auto max-w-7xl px-4 sm:px-6">
          <div className="max-w-2xl">
            <span className="eyebrow text-ink">Inside the Webinar</span>
          ...
          <div className="mt-12 grid gap-5 lg:grid-cols-3">
            ...
            <BentoCard className="lg:col-span-2">
              ...
              <ul className="mt-6 grid gap-3 sm:grid-cols-2">
                {((track as any).topics || []).map((topic: any, i: number) => {
                  const Icon = getIcon(topic.icon || "Code");
                  return (
                    <motion.li
                      key={topic.title}
            ...
            <BentoCard tone="yellow">
              ...
              <ul className="mt-5 space-y-4">
                {((track as any).outcomes || []).map((o: string, i: number) => (
            ...
            <BentoCard tone="ink" className="lg:col-span-3">
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {((track as any).agenda || []).map((a: any, i: number) => (
                  <div
                    key={i}
      ...
      <section className="bg-transparent py-20">
        <div className="container mx-auto max-w-7xl px-4 sm:px-6">
          <div className="mb-10 flex items-end justify-between">
            <h3 className="font-display text-3xl font-bold text-ink">Other webinars</h3>
            <Link
              to="/webinars"
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {tracks
              .filter((t: Webinar) => (t.id || t.title) !== ((track as any).slug || track.id || track.title))
              .map((t: Webinar) => (
                <Link
                  key={t.id || t.title}
                  to="/webinars/$slug"
                  params={{ slug: t.id || t.title }}
