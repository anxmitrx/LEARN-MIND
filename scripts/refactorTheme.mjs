...
content = content.replace(
  /min-h-screen bg-transparent text-slate-800 dot-bg/g,
  "min-h-screen bg-[#0B0F19] text-slate-300 relative overflow-hidden",
);

// Gradients for Hero
content = content.replace(
  /from-\[#F8EDEB\] via-\[#F8EDEB\]\/60/g,
  "from-[#0B0F19] via-[#0B0F19]/80",
);
content = content.replace(
  /sm:from-\[#f8f5ff\]\/95 sm:via-\[#f8f5ff\]\/70/g,
  "sm:from-[#0B0F19]/95 sm:via-[#0B0F19]/80",
);
...
// Shadows
content = content.replace(
  /shadow-\[0_8px_32px_0_rgba\(31,38,135,0\.07\)\]/g,
